...
use crate::{
    ...
    mcp::{
        ...
        tools::{
            ...
            RunResponse, StartRequest, StartResponse, TargetProbe, TargetRequest,
        },
    },
    runner::{
...
#[tool_router]
impl RemoteOpsService {
    #[tool(description = "Report local backend capability for ssh, rsync, sftp and scp")]
    ...
    async fn remote_target_probe(
        ...
        let result = async {
            let target = resolve_target(&self.state.config, request.target)?;
    ...
    async fn remote_run(&self, Parameters(request): Parameters<RunRequest>) -> McpToolResult {
        let result = async {
            ...
            let target = resolve_target(&self.state.config, request.target.clone())?;
            let timeout_s = request
                .timeout_s
                .unwrap_or(self.state.config.defaults.run_timeout_s);
        ...
        let result = async {
            ...
            let initial = self
                ...
                .follow(
                    ...
                    request
                        .follow_limit
                        .unwrap_or(self.state.config.defaults.follow_limit),
                    request
                        .initial_wait_s
                        .unwrap_or(self.state.config.defaults.initial_wait_s),
                )
                .await?;
    ...
    async fn remote_follow(&self, Parameters(request): Parameters<FollowRequest>) -> McpToolResult {
        let result = self
            ...
            .follow(
                ...
                request
                    .wait_s
                    .unwrap_or(self.state.config.defaults.follow_wait_s),
            )
            .await;
